...
def calc_H_D(trainLabelArr):
    ...
    for i in trainLabelSet:
        ...
        p = trainLabelArr[trainLabelArr == i].size / trainLabelArr.size # 计算每个类别的概率(频率)
        # 对经验熵的每一项累加求和
        H_D += -1 * p * np.log2(p)
...
def calcBestFeature(trainDataList, trainLabelList):
    ...
    featureNum = trainDataArr.shape[1] # featureNum = 784
    ...
    for feature in range(featureNum):
        ...
        # Arr[:, feature]这么一竖条来，因为后续计算中数据集中只用到这个（没明白的跟着算一遍例5.2）
...
def createTree(*dataSet):
    ...
    # 之所以使用元组作为参数，是由于后续递归调用时，数据集需要对某个特征进行切割，在函数递归
    # 调用上直接将切割函数的返回值放入递归调用中，而函数的返回值形式是元组的，等看到这个函数
...
def predict(testDataList, tree):
    ...
    while True:
        ...
        key = list(tree.keys())[0]  # 树根
        value = list(tree.values())[0] # 树根的左右分支
